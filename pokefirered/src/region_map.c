...
extern u16 gTrackerSelectedSpecies;
static void CreateTrackerMonIcons(void);
static void FreeTrackerMonIcons(void);
static void UpdateTrackerMonIconVisibility(void);
...
static void FreeSwitchMapMenu(u8 taskId)
{
    ...
    DrawDungeonNameBox();
    SetGpuWindowDims(0, &sMapsecNameWindowDims[CLEAR_NAME]);
    UpdateTrackerMonIconVisibility();
...
static EWRAM_DATA u8 sTrackerMonIconSpriteIds[32] = {0};
static EWRAM_DATA u8 sTrackerMonIconRegions[32] = {0};
static EWRAM_DATA u8 sTrackerMonIconCount = 0;
...
static const struct StaticLocation sStaticLocations[] = {
        { SPECIES_BULBASAUR, MAPSEC_PALLET_TOWN },
    { SPECIES_IVYSAUR, MAPSEC_PALLET_TOWN },
    { SPECIES_VENUSAUR, MAPSEC_PALLET_TOWN },
    { SPECIES_CHARMANDER, MAPSEC_PALLET_TOWN },
    { SPECIES_CHARMELEON, MAPSEC_PALLET_TOWN },
    { SPECIES_CHARIZARD, MAPSEC_PALLET_TOWN },
    { SPECIES_SQUIRTLE, MAPSEC_PALLET_TOWN },
    { SPECIES_WARTORTLE, MAPSEC_PALLET_TOWN },
    { SPECIES_BLASTOISE, MAPSEC_PALLET_TOWN },
    { SPECIES_HITMONLEE, MAPSEC_SAFFRON_CITY },
    { SPECIES_HITMONCHAN, MAPSEC_SAFFRON_CITY },
    { SPECIES_LAPRAS, MAPSEC_SAFFRON_CITY },
    { SPECIES_EEVEE, MAPSEC_CELADON_CITY },
    { SPECIES_VAPOREON, MAPSEC_CELADON_CITY },
    { SPECIES_JOLTEON, MAPSEC_CELADON_CITY },
    { SPECIES_FLAREON, MAPSEC_CELADON_CITY },
    { SPECIES_OMANYTE, MAPSEC_CINNABAR_ISLAND },
    { SPECIES_OMASTAR, MAPSEC_CINNABAR_ISLAND },
    { SPECIES_KABUTO, MAPSEC_CINNABAR_ISLAND },
    { SPECIES_KABUTOPS, MAPSEC_CINNABAR_ISLAND },
    { SPECIES_AERODACTYL, MAPSEC_CINNABAR_ISLAND },
    { SPECIES_FARFETCHD, MAPSEC_VERMILION_CITY },
    { SPECIES_LICKITUNG, MAPSEC_ROUTE_18 },
    { SPECIES_MR_MIME, MAPSEC_ROUTE_2 },
    { SPECIES_JYNX, MAPSEC_CERULEAN_CITY },
    { SPECIES_ARTICUNO, MAPSEC_SEAFOAM_ISLANDS },
    { SPECIES_ZAPDOS, MAPSEC_POWER_PLANT },
    { SPECIES_MOLTRES, MAPSEC_MT_EMBER },
    { SPECIES_MEWTWO, MAPSEC_CERULEAN_CAVE },
    { SPECIES_LUGIA, MAPSEC_NAVEL_ROCK },
    { SPECIES_HO_OH, MAPSEC_NAVEL_ROCK },
    { SPECIES_DEOXYS, MAPSEC_BIRTH_ISLAND },
};

static bool32 IsSpeciesInEncounterTable(const struct WildPokemonInfo*info, u16 species, s32 count){
    s32 i;
    if(info != NULL){
        for(i = 0; i < count; i++){
...
static bool32 IsSpeciesOnMap(const struct WildPokemonHeader *header, u16 species){
    if(IsSpeciesInEncounterTable(header->landMonsInfo, species, 12))
    return TRUE;
    if(IsSpeciesInEncounterTable(header->waterMonsInfo, species, 5))
    return TRUE;
    if(IsSpeciesInEncounterTable(header->fishingMonsInfo, species, 10))
    return TRUE;
    if(IsSpeciesInEncounterTable(header->rockSmashMonsInfo, species, 5))
    return TRUE;
...
static bool32 GetMapSecCoordsAndRegion(u8 mapSecId, s16 *gridX, s16 *gridY, u8 *region){
    s32 y, x;
    
    // Check Kanto
    for(y = 0; y < MAP_HEIGHT; y++){
        for(x = 0; x < MAP_WIDTH; x++){
            if(sRegionMapSections_Kanto[LAYER_MAP][y][x] == mapSecId ||
               sRegionMapSections_Kanto[LAYER_DUNGEON][y][x] == mapSecId){
                *gridX = x;
                *gridY = y;
                *region = REGIONMAP_KANTO;
                return TRUE;
    ...
    for(y = 0; y < MAP_HEIGHT; y++){
        for(x = 0; x < MAP_WIDTH; x++){
            if(sRegionMapSections_Sevii123[LAYER_MAP][y][x] == mapSecId ||
               sRegionMapSections_Sevii123[LAYER_DUNGEON][y][x] == mapSecId){
                *gridX = x;
                *gridY = y;
    ...
    for(y = 0; y < MAP_HEIGHT; y++){
        for(x = 0; x < MAP_WIDTH; x++){
            if(sRegionMapSections_Sevii45[LAYER_MAP][y][x] == mapSecId ||
               sRegionMapSections_Sevii45[LAYER_DUNGEON][y][x] == mapSecId){
                *gridX = x;
                *gridY = y;
    ...
    for(y = 0; y < MAP_HEIGHT; y++){
        for(x = 0; x < MAP_WIDTH; x++){
            if(sRegionMapSections_Sevii67[LAYER_MAP][y][x] == mapSecId ||
               sRegionMapSections_Sevii67[LAYER_DUNGEON][y][x] == mapSecId){
                *gridX = x;
                *gridY = y;
...
{
    s32 i;
    u8 currentRegion = sRegionMap->selectedRegion;
    for (i = 0; i < sTrackerMonIconCount; i++)
    {
        if (sTrackerMonIconRegions[i] == currentRegion)
            gSprites[sTrackerMonIconSpriteIds[i]].invisible = FALSE;
        else
            gSprites[sTrackerMonIconSpriteIds[i]].invisible = TRUE;
    }
}
...
static void CreateTrackerMonIcons(void){
    ...
    sTrackerMonIconCount = 0;
    if(gTrackerSelectedSpecies == SPECIES_NONE)
    return;

    if(gTrackerSelectedSpecies == SPECIES_SNORLAX){
        mapSecs[count++] = MAPSEC_ROUTE_12;
        mapSecs[count++] = MAPSEC_ROUTE_16;
    }
    else if (gTrackerSelectedSpecies == SPECIES_RAIKOU || gTrackerSelectedSpecies == SPECIES_ENTEI || gTrackerSelectedSpecies == SPECIES_SUICUNE){
        mapSecs[count++] = MAPSEC_ROUTE_1;
    } else{
        for(i = 0; i < ARRAY_COUNT(sStaticLocations); i++){
            if(sStaticLocations[i].species == gTrackerSelectedSpecies){
                mapSecs[count++] = sStaticLocations[i].mapSecId;
                break;
            }
        }
        if(count == 0){
            for(i = 0; gWildMonHeaders[i].mapGroup != 0xFF; i++){
                if(IsSpeciesOnMap(&gWildMonHeaders[i], gTrackerSelectedSpecies)){
                    ...
                    for(j = 0; j < count; j++){
                        if(mapSecs[j] == mapSecId){
                            duplicate = TRUE;
                            break;
                        }
                    }
                    if(!duplicate){
                        mapSecs[count++] = mapSecId;
                        if(count >= 32)
                        break;
                    }
                }
    ...
    if(count > 0){
        ...
        for(i = 0; i < count; i++){
            s16 gridX = 0, gridY = 0;
            u8 region = 0;
            if(GetMapSecCoordsAndRegion(mapSecs[i], &gridX, &gridY, &region)){
                s16 pixelX = 8 * gridX + 36;
                s16 pixelY = 8 * gridY + 36;
                u8 spriteId = CreateMonIcon(gTrackerSelectedSpecies, SpriteCallbackDummy, pixelX, pixelY, 0, 0, TRUE);
                if(spriteId != MAX_SPRITES){
                    sTrackerMonIconSpriteIds[sTrackerMonIconCount] = spriteId;
                    sTrackerMonIconRegions[sTrackerMonIconCount] = region;
                    sTrackerMonIconCount++;
...
static void FreeTrackerMonIcons(void){
    s32 i;
    for(i = 0; i < sTrackerMonIconCount; i++){
        DestroyMonIcon(&gSprites[sTrackerMonIconSpriteIds[i]]);
    }
    sTrackerMonIconCount = 0;
    FreeMonIconPalettes();
}
