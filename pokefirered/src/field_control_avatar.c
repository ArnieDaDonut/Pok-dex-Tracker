...
static void Task_InitTrackerFromField(u8 taskId);
...
static void QuestLogOverrideJoyVars(struct FieldInput *input, u16 *newKeys,
                                    u16 *heldKeys);
...
static const u8 *GetInteractionScript(struct MapPosition *position,
                                      u8 metatileBehavior, u8 playerDirection);
...
static const u8 *GetInteractedMetatileScript(struct MapPosition *position,
                                             u8 metatileBehavior,
                                             u8 playerDirection);
static const u8 *GetInteractedWaterScript(struct MapPosition *position,
                                          u8 metatileBehavior,
                                          u8 playerDirection);
static bool8 TryStartStepBasedScript(struct MapPosition *position,
                                     u16 metatileBehavior, u16 playerDirection);
...
static bool8 TrySetUpWalkIntoSignpostScript(struct MapPosition *position,
                                            u16 metatileBehavior,
                                            u8 playerDirection);
...
static void SetupWarp(struct MapHeader *mapHeader, s8 warpId,
...
void FieldGetPlayerInput(struct FieldInput *input, u16 newKeys, u16 heldKeys) {
  ...
  bool8 forcedMove =
      MetatileBehavior_IsForcedMovementTile(GetPlayerCurMetatileBehavior());
...
int ProcessPlayerFieldInput(struct FieldInput *input) {
  ...
  metatileAttributes = MapGridGetMetatileAttributeAt(position.x, position.y,
  ...
  if (input->checkStandardWildEncounter) {
    if (input->dpadDirection == 0 || input->dpadDirection == playerDirection) {
      ...
      metatileBehavior = MapGridGetMetatileBehaviorAt(position.x, position.y);
      if (TrySetUpWalkIntoSignpostScript(&position, metatileBehavior,
  ...
  if (input->checkStandardWildEncounter &&
      CheckStandardWildEncounter(metatileAttributes) == TRUE) {
    gFieldInputRecord.checkStandardWildEncounter = TRUE;
    return TRUE;
  }
  ...
  if (((gMain.heldKeys & B_BUTTON) && input->pressedSelectButton) ||
      ((gMain.heldKeys & SELECT_BUTTON) && input->pressedBButton)) {
    PlaySE(SE_WIN_OPEN);
    FreezeObjectEvents();
    LockPlayerFieldControls();
    FadeScreen(FADE_TO_BLACK, 0);
    CreateTask(Task_InitTrackerFromField, 8);
    return TRUE;
  }
...
static void Task_InitTrackerFromField(u8 taskId) {
  if (!gPaletteFade.active) {
    ...
    SetMainCallback2(CB2_InitTrackerScreen);
    DestroyTask(taskId);
  }
}
  ...
  if (ScriptContext_IsEnabled() == TRUE) {
    if (gWalkAwayFromSignInhibitTimer != 0)
      ...
      if (input->dpadDirection != 0 &&
          GetPlayerFacingDirection() != input->dpadDirection) {
...
static const u8 *GetInteractedObjectEventScript(struct MapPosition *position,
  ...
  if (InUnionRoom() == TRUE &&
      !ObjectEventCheckHeldMovementStatus(&gObjectEvents[objectEventId]))
    return NULL;
...
GetInteractedBackgroundEventScript(struct MapPosition *position,
  ...
  case BG_EVENT_HIDDEN_ITEM:
    if (GetHiddenItemAttr(bgEvent->bgUnion.hiddenItem, HIDDEN_ITEM_UNDERFOOT) ==
        TRUE)
      return NULL;
    gSpecialVar_0x8005 =
        GetHiddenItemAttr(bgEvent->bgUnion.hiddenItem, HIDDEN_ITEM_ITEM);
    gSpecialVar_0x8004 =
        GetHiddenItemAttr(bgEvent->bgUnion.hiddenItem, HIDDEN_ITEM_FLAG);
    gSpecialVar_0x8006 =
        GetHiddenItemAttr(bgEvent->bgUnion.hiddenItem, HIDDEN_ITEM_QUANTITY);
...
static const u8 *GetInteractedMetatileScript(struct MapPosition *position,
  ...
  if (MetatileBehavior_IsPlayerFacingCableClubWirelessMonitor(
          metatileBehavior, direction) == TRUE)
  ...
  if (MetatileBehavior_IsPlayerFacingBattleRecords(metatileBehavior,
                                                   direction) == TRUE)
  ...
  if (MetatileBehavior_IsPlayerFacingPokeMartSign(metatileBehavior,
                                                  direction) == TRUE) {
    MsgSetSignpost();
    return EventScript_PokemartSign;
  }
  if (MetatileBehavior_IsPlayerFacingPokemonCenterSign(metatileBehavior,
                                                       direction) == TRUE) {
    MsgSetSignpost();
...
void RestartWildEncounterImmunitySteps(void) { ResetEncounterRateModifiers(); }
...
static bool8 TrySetUpWalkIntoSignpostScript(struct MapPosition *position,
  ...
  if (signpostType == SIGNPOST_POKECENTER) {
    ...
    SetUpWalkIntoSignScript(EventScript_Indigo_HighestAuthority,
                            playerDirection);
    return TRUE;
  } else {
...
static const u8 *GetSignpostScriptAtMapPosition(struct MapPosition *position) {
  const struct BgEvent *event = GetBackgroundEventAtPosition(
      &gMapHeader, position->x - MAP_OFFSET, position->y - MAP_OFFSET,
      position->elevation);
  if (event == NULL)
    return NULL;
...
static bool8 TryArrowWarp(struct MapPosition *position, u16 metatileBehavior,
  ...
  if (warpEventId != -1) {
    if (IsArrowWarpMetatileBehavior(metatileBehavior, direction) == TRUE) {
      ...
      DoWarp();
      return TRUE;
    } else if (IsDirectionalStairWarpMetatileBehavior(metatileBehavior,
                                                      direction) == TRUE) {
      delay = 0;
...
bool8 IsDirectionalStairWarpMetatileBehavior(u16 metatileBehavior,
                                             u8 playerDirection) {
...
static void SetupWarp(struct MapHeader *unused, s8 warpEventId,
  ...
  if (warpEvent->mapNum == MAP_NUM(MAP_DYNAMIC)) {
    ...
    mapHeader = Overworld_GetMapHeaderByGroupAndId(warpEvent->mapGroup,
                                                   warpEvent->mapNum);
    if (mapHeader->events->warps[warpEvent->warpId].mapNum ==
        MAP_NUM(MAP_DYNAMIC))
...
void HandleBoulderFallThroughHole(struct ObjectEvent *object) {
  if (MapGridGetMetatileBehaviorAt(object->currentCoords.x,
                                   object->currentCoords.y) == MB_FALL_WARP) {
    PlaySE(SE_FALL);
    RemoveObjectEventByLocalIdAndMap(object->localId,
                                     gSaveBlock1Ptr->location.mapNum,
                                     gSaveBlock1Ptr->location.mapGroup);
    FlagClear(GetBoulderRevealFlagByLocalIdAndMap(
        object->localId, gSaveBlock1Ptr->location.mapNum,
        gSaveBlock1Ptr->location.mapGroup));
  }
}
